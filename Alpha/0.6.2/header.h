...
<<<<<<< HEAD
void getAllInformation(Mat firstsrc,Target& target,int left,int right)
{
    const int rows = firstsrc.rows;

    Mat firstleft = Mat(firstsrc,Rect(0,0,left,rows));
    Mat firstright = Mat(firstsrc,Rect(right,0,640-right,rows));
...
<<<<<<< HEAD
int getAim(Mat firstsrc,int left,int right)   //总流程函数
{
    const int rows = firstsrc.rows;
