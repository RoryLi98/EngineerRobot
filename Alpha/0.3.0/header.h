...
//    Mat secondmid = Mat(secondsrc,Rect(a,0,b-a,rows));
    ...
    if(ifbox(firstleft))  target.position1 = 1;
    if(ifbox(firstright)) target.position3 = 1;

    if(ifbox(secondleft)) target.position4 = 1;
    if(ifbox(secondright)) target.position6 = 1;
