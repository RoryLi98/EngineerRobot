...
int main()
{
    ...
    while (true)
    {
        ...
        data_send[0].data_float = int(cnt);                //数据装载
        data_send[1].data_float = 0;                     //数据装载
        data_send[2].data_float = 0;                     //数据装载
        HotSwap = sendXYZ(fd2car, data_send, 0, 3, cnt); //发送数据
        cnt = (cnt + 1) % 255;                           //累加
        continue;
        ...
        else if (mode == 3)
        {
            ...
            data_send[2].data_float = 0;                                //数据装载
            HotSwap = sendXYZ(fd2car, data_send, AbleToSwitch, 2, cnt); //发送数据
            cnt = (cnt + 1) % 255;                                      //累加
            continue;
        }
        else if (mode == 1) //台上抓取
        ...
        HotSwap = sendXYZ(fd2car, data_send, AbleToGrab, 1, cnt); //发送数据
        cnt = (cnt + 1) % 255;                                    //累加
        //------------------------------------------------------------------
    }
