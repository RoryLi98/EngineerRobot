...
//    morphologyEx(image, image, MORPH_OPEN, element);//闭运算滤波
    ...
    vector<vector<Point2f>> DoubleRectangles;
...
//          cout<<abs((TempPoint[(MinPointNum + 1) % 3].y- TempPoint[MinPointNum].y)/(TempPoint[(MinPointNum + 1) % 3].x- TempPoint[MinPointNum].x))<<endl;
      ...
      if(PointSize==4)
      {
        ...
        Point2f KeyPoint1 = Point(-1,-1);
        Point2f KeyPoint2 = Point(-1,-1);
        Point2f MinPointA = Point(-1,-1);
        Point2f MinPointB = Point(-1,-1);
        if(DoubleRectangles.size()>2 && Squares.size()>0)
        {
            ...
            for(int i = 0;i<DoubleRectangles.size()-2;i++)
            {
              ...
              {
                  KeyPoint1 = MinPoint;
                  circle(img, KeyPoint1 , 3, Scalar(106,68,20), 3, CV_AA, 0);
                  float Verticalrate = 8888;
                  Point2f BestPoint = Point(-1,-1);
                  for(int i=0;i<Squares.size()>0;i++)
                  {
                      float k1tok2Max = -1;
                      Point2f MaxPoint = Point(-1,-1);
                      for(int j=0;j<4;j++)
                      {
...
//                      circle(img, MaxPoint , 5, Scalar(0,0,0), 5, CV_AA, 0);
                  ...
                  if(Verticalrate<8000)
                  {
                      KeyPoint2 = BestPoint;
                      circle(img, KeyPoint2 , 3, Scalar(106,68,20), 3, CV_AA, 0);
                  }
